...
import {
  archiveDoneTickets,
  completeTicket,
} from "../../application/tickets/archive-tickets.js";
...
export async function runTicketsCommand(
  action: string,
  args: string[],
): Promise<void> {
  ...
  if (action === "doctor") {
    try {
      const result = await execFile(process.execPath, [
        enginePath("scripts", "validate-tickets.mjs"),
        resolveWithin(atlasPath("projects"), projectId, "tickets"),
      ]);
  ...
  if (action === "index") {
    ...
    const body = [
      "# Tickets",
      "",
      "<!-- Generated by `atlas tickets index --write`. Do not edit by hand. -->",
      "",
      "| ID | State | Title | Goal |",
      "|---|---|---|---|",
      ...tickets.map(
        (ticket) =>
          `| ${ticket.id} | ${ticket.state} | ${ticket.title.replaceAll("|", "\\|")} | ${ticket.goal.replaceAll("|", "\\|")} |`,
      ),
      "",
    ].join("\n");
  ...
  if (action === "checkpoint") {
    ...
    if (!id || noteIndex < 0 || !args[noteIndex + 1])
      throw new Error(
        "Usage: atlas tickets checkpoint <id> --note <text> [--next <text>] [--project <id>]",
      );
  ...
  if (action !== "archive") {
    console.error(
      "Usage: atlas tickets list [state] [--project <id>]|doctor|index [--write]|checkpoint <id> --note <text> [--next <text>]|complete <id>|archive [--apply|--auto]",
    );
    process.exitCode = 1;
    return;
  }
  const apply = args.includes("--apply") || args.includes("--auto");
  const result = await archiveDoneTickets(
    resolveWithin(atlasPath("projects"), projectId, "tickets"),
    apply,
  );
