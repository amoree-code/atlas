...
export async function runContextCommand(json = false): Promise<void> {
  ...
  if (resolution.status !== "bound") console.log(`project: ${resolution.status} — no Atlas binding for this directory; run 'atlas project bind <name> <path>'`);
  else console.log(`project: ${resolution.projectId} (confidence: ${resolution.confidence})`);
