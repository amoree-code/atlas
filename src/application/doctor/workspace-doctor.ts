...
async function activeDirectories(directory: string): Promise<string[]> {
  ...
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) directories.push(...await activeDirectories(path.join(directory, entry.name)));
  }
  return directories;
}
...
async function checkStructure(): Promise<Finding[]> {
  ...
  for (const root of roots) {
    ...
    findings.push(await exists(directory)
      ? { code: "ROOT_PRESENT", severity: "OK", path: root, message: `${root} root exists`, fixable: false }
      : { code: "ROOT_MISSING", severity: "FAIL", path: root, message: `${root} root is missing`, fixable: false });
  }
  return findings;
}
  ...
  for (const root of roots) {
    for (const file of await markdownFiles(atlasPath(root))) {
      ...
      for (const link of links(content)) {
        ...
        if (!(await exists(target))) findings.push({
          code: "BROKEN_LINK", severity: "WARN", path: path.relative(atlasPath(), file),
          message: `broken relative link: ${link}`, fixable: false,
        });
      }
    }
  }
...
async function checkWrappers(): Promise<Finding[]> {
  ...
  return errors.length
    ? errors.map((message) => ({ code: "WRAPPER_DRIFT", severity: "WARN" as const, message, fixable: /wrapper is (stale|missing)/i.test(message) }))
    : [{ code: "WRAPPERS_READY", severity: "OK", message: "Atlas wrappers are synchronized", fixable: false }];
}
...
async function checkWorkspaceContracts(): Promise<Finding[]> {
  const checks = [
    ...
    ["TICKET_RECORDS", "node", ["scripts/validate-tickets.mjs", atlasPath("projects", "atlas", "tickets")]],
  ] as const;
  const findings: Finding[] = [];
...
async function checkPermissions(): Promise<Finding[]> {
  if (process.platform === "win32") return [{ code: "PERMISSIONS_SKIPPED", severity: "OK", message: "owner-only directory mode is not applicable on Windows", fixable: false }];
  const unsafe: string[] = [];
  for (const directory of await activeDirectories(atlasPath("system"))) {
    const mode = (await stat(directory)).mode & 0o777;
    if ((mode & 0o077) !== 0) unsafe.push(path.relative(atlasPath(), directory));
  }
  return unsafe.length
    ? [{ code: "PRIVATE_PERMISSIONS", severity: "WARN", message: `system directories are not owner-only: ${unsafe.join(", ")}`, fixable: true }]
    : [{ code: "PRIVATE_PERMISSIONS", severity: "OK", message: "active system directories are owner-only", fixable: false }];
}
...
async function checkDuplicates(): Promise<Finding[]> {
  ...
  return duplicates.length
    ? [{ code: "DUPLICATE_RECORDS", severity: "WARN", message: `identical active markdown files: ${duplicates.join("; ")}`, fixable: false }]
...
export async function repairWorkspace(): Promise<{ changes: string[]; findings: Finding[] }> {
  ...
  if (before.some((finding) => finding.code === "PRIVATE_PERMISSIONS" && finding.fixable)) {
    for (const directory of await activeDirectories(atlasPath("system"))) await chmod(directory, 0o700);
  ...
  await writeFile(path.join(reportDirectory, "repair-report.json"), `${JSON.stringify({ changes, before, after }, null, 2)}\n`);
  return { changes, findings: after };
}

export async function workspaceReport(): Promise<{ findings: Finding[]; generatedAt: string }> {
  return { findings: await scanWorkspace(), generatedAt: new Date().toISOString() };
}
