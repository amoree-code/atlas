...
const dailyTemplate = (date: string): string => `# Daily — ${date}\n\n## Focus\n\n## Work log\n\n## Decisions\n\n## Problems\n\n## Next\n`;
 ...
 */
export async function appendDailyNarrative(input: { session: Session; events: SessionEvent[]; exitCode?: number; nextAction?: string; narrative?: ModelNarrative | null }): Promise<void> {
  ...
  catch { content = dailyTemplate(date); }

  const time = new Date().toISOString().slice(11, 16);
  const project = path.basename(session.workingDirectory) || session.workingDirectory;
  const status = session.status === "completed" && (input.exitCode ?? 0) === 0 ? "completed" : "failed";

  const workLog = narrative?.workLog ?? (() => {
    const objectiveEvent = events.find((event) => event.type === "user_input" || event.type === "resume_requested");
    return safeText(objectiveEvent?.data ?? session.title, 160);
  })();
  content = insertUnderHeading(content, "## Work log", `- ${time} ${session.provider} session in ${project}: ${workLog} — ${status}`);

  const decisionLine = narrative?.decision ?? events
  ...
  if (decisionLine) content = insertUnderHeading(content, "## Decisions", `- ${decisionLine}`);

  const problemLine = narrative?.problem ?? events
    .filter((event) => event.type === "error" || event.type === "provider_blocked")
    .map((event) => safeText(event.data, 500))[0];
  if (problemLine) content = insertUnderHeading(content, "## Problems", `- ${problemLine}`);

  const nextAction = narrative?.next ?? input.nextAction ?? session.nextAction;
  if (nextAction && nextAction !== "Review the summary and verify the next action.") {
    content = insertUnderHeading(content, "## Next", `- [${project}] ${safeText(nextAction, 240)}`);
  }
