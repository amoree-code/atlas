...
const kinds = new Set(["architecture", "decisions", "discoveries", "failures", "research", "results", "solutions", "references"]);

export async function promoteSessionToKnowledge(sessionId: string, target = "knowledge/results", approved = false): Promise<{ applied: boolean; sessionId: string; file?: string }> {
  if (!approved) throw new Error("Session promotion requires explicit approval: use --approve or approved: true");
  const match = /^knowledge\/(\w+)$/.exec(target);
  if (!match || !kinds.has(match[1])) throw new Error("Target must be knowledge/<architecture|decisions|discoveries|failures|research|results|solutions|references>");
  ...
  try {
    ...
    const slug = `${session.provider}-${sessionId.slice(0, 8)}`.replace(/[^a-zA-Z0-9-]+/g, "-");
    const file = path.join(atlasPath("personal", "knowledge", match[1], `${slug}.md`));
    ...
    store.appendEvent(sessionId, "knowledge_promoted", JSON.stringify({ file: path.relative(atlasPath("personal", "knowledge"), file), target, approved: true }));
