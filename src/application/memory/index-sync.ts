...
function title(content: string, fallback: string): string {
  ...
  const declared = frontmatter.match(/^(?:title|name):\s*["']?(.+?)["']?\s*$/m)?.[1]?.trim();
  if (declared) return declared;
  const heading = content.match(/^#\s+(.+)$/m)?.[1]?.trim();
  if (heading && !/^(decision|discovery|failure|solution|research|result|architecture)$/i.test(heading)) return heading;
  const summary = content.replace(/^---[\s\S]*?---\s*/m, "").split("\n").map((line) => line.trim())
  ...
  return fallback.split("/").at(-1)?.replace(/\.md$/, "").replace(/[-_]+/g, " ") ?? fallback;
}
...
async function syncStore(store: Store, apply: boolean) {
  ...
  const recordFiles = (await markdownFiles(root)).filter((file) => path.basename(file) !== indexName);
  const known = links(current);
  const missing = recordFiles.filter((file) => !known.has(relativeRecord(root, file)));
  const broken: string[] = [];
  for (const link of known) {
    if (!link.startsWith("http") && !(await awaitedExists(path.resolve(root, link)))) broken.push(link);
  }
  if (!apply) return { store, missing: missing.map((file) => relativeRecord(root, file)), broken, changed: false };

  const generated = [generatedStart, "", ...recordFiles.map((file) => {
  ...
  return { store, missing: missing.map((file) => relativeRecord(root, file)), broken, changed };
}
...
export async function syncMemoryIndexes(apply: boolean) {
  return Promise.all([syncStore("memory", apply), syncStore("knowledge", apply)]);
}

export async function doctorMemoryIndexes() {
  const results = await syncMemoryIndexes(false);
  const missing = results.flatMap((result) => result.missing.map((file) => `${result.store}/${file}`));
  const broken = results.flatMap((result) => result.broken.map((file) => `${result.store}/${file}`));
