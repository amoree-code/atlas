...
export async function configureClaudeCodeWrapper(settingsPath = defaultClaudeCodeSettingsPath(), apply = false): Promise<{
  settingsPath: string;
  wrapper: string;
  apply: boolean;
  changed: boolean;
  ...
  let source = "{}\n";
  try { source = await readFile(settingsPath, "utf8"); } catch (error) {
    if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
  }
  let settings: Record<string, unknown>;
  try { settings = JSON.parse(source) as Record<string, unknown>; }
  catch { throw new Error(`Claude Code settings must be plain JSON for automatic setup: ${settingsPath}`); }
  const changed = settings["claudeCode.claudeProcessWrapper"] !== wrapper;
  if (!apply || !changed) return { settingsPath, wrapper, apply, changed, backup: null };
  ...
  try {
    ...
    await writeFile(settingsPath, `${JSON.stringify({ ...settings, "claudeCode.claudeProcessWrapper": wrapper }, null, 2)}\n`, "utf8");
