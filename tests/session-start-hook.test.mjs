...
import { claudeNativeHookStatus, claudeSessionStartHook } from "../dist/application/hooks/session-start-hook.js";
...
test("claudeSessionStartHook returns the documented Claude Code hookSpecificOutput shape", async () => {
  ...
  assert.ok(Buffer.byteLength(result.hookSpecificOutput.additionalContext) <= ATLAS_BOOTSTRAP_MAX_BYTES);
...
test("atlas hook session-start CLI: bounded stdout JSON, run from a cwd outside Atlas, no Atlas file content", () => {
  ...
  const payload = JSON.stringify({ cwd: outside, session_id: "cli-test", hook_event_name: "SessionStart" });
  const result = spawnSync(process.execPath, [path.resolve("dist/main.js"), "hook", "session-start"], { input: payload, encoding: "utf8" });
  assert.equal(result.status, 0, result.stderr);
  const parsed = JSON.parse(result.stdout.trim());
  assert.equal(parsed.hookSpecificOutput.hookEventName, "SessionStart");
  assert.ok(Buffer.byteLength(parsed.hookSpecificOutput.additionalContext) <= 256);
  assert.doesNotMatch(parsed.hookSpecificOutput.additionalContext, /MEMORY|KNOWLEDGE|## Atlas resource/);
});

test("atlas hook session-start CLI works with no stdin payload at all (falls back to process cwd)", () => {
  const result = spawnSync(process.execPath, [path.resolve("dist/main.js"), "hook", "session-start"], { input: "", encoding: "utf8" });
  assert.equal(result.status, 0, result.stderr);
  const parsed = JSON.parse(result.stdout.trim());
  assert.equal(parsed.hookSpecificOutput.hookEventName, "SessionStart");
});
...
test("claudeNativeHookStatus reports installed-but-not-registered when the script exists but settings.json does not reference it", async () => {
  const fakeHome = await mkdtemp(path.join(os.tmpdir(), "atlas-fake-home-"));
  await mkdir(path.join(fakeHome, "atlas", "system", "integrations", "claude-code", "hooks"), { recursive: true });
  await writeFile(path.join(fakeHome, "atlas", "system", "integrations", "claude-code", "hooks", "atlas-session-bootstrap"), "#!/bin/sh\n");
  await mkdir(path.join(fakeHome, ".claude"), { recursive: true });
  await writeFile(path.join(fakeHome, ".claude", "settings.json"), JSON.stringify({ hooks: { SessionStart: [] } }));
  const status = await claudeNativeHookStatus(fakeHome);
...
test("claudeNativeHookStatus reports registered only when settings.json actually references the script", async () => {
  const fakeHome = await mkdtemp(path.join(os.tmpdir(), "atlas-fake-home-"));
  await mkdir(path.join(fakeHome, "atlas", "system", "integrations", "claude-code", "hooks"), { recursive: true });
  const scriptPath = path.join(fakeHome, "atlas", "system", "integrations", "claude-code", "hooks", "atlas-session-bootstrap");
  await writeFile(scriptPath, "#!/bin/sh\n");
  await mkdir(path.join(fakeHome, ".claude"), { recursive: true });
  await writeFile(path.join(fakeHome, ".claude", "settings.json"), JSON.stringify({
