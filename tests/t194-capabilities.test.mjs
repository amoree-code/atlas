...
import { observeSession, listObservations, reviewObservation } from "../dist/application/skills/task-observer.js";
...
test("default skill policy stays small and verification-first", () => {
  assert.deepEqual(defaultSkillNames("general assistant"), ["core-thinking", "verification"]);
  assert.deepEqual(defaultSkillNames("developer"), ["core-thinking", "verification"]);
});
...
test("compression preserves required evidence and records recovery metadata", () => {
  const source = [
    ...
    ...Array.from({ length: 80 }, (_, index) => `repeated log line ${index % 4}`),
    "next action: inspect the remaining failure",
  ].join("\n");
  const result = compressContext({ sourceId: "tool-1", content: source, budget: 260 });
...
test("compression falls back to bounded original when required evidence cannot fit safely", () => {
  const result = compressContext({ sourceId: "tiny", content: "security warning: keep this", budget: 8 });
...
test("profile-scoped compression keeps context within budget and records a benchmark", async () => {
  ...
  const content = ["tool json: {\"items\":[1,2,3]}", "verification: passed", ...Array.from({ length: 100 }, () => "repeated log output")].join("\n");
  await mkdir(path.join(root, "context"), { recursive: true });
  await writeFile(path.join(root, "context", "output.txt"), content);
  const result = await buildContext({ name: "benchmark", description: "", version: "1", provider: "claude", model: "managed", role: "developer", skills: [], allowedPaths: ["context"], allowedCommands: [], writePolicy: "none", contextSources: ["context/output.txt"], clients: { claude: { enabled: true, capabilities: [], limitations: [] } }, defaultClient: "claude", memory: { enabled: true, scope: "profile" }, verification: { commands: [] }, instructions: "", contextCompression: "atlas-bounded" }, root, 220, { compression: "atlas-bounded" });
  assert.equal(result.manifest.compression.method, "atlas-bounded-v1");
  assert.ok(result.manifest.compression.originalBytes > result.manifest.compression.compressedBytes);
  assert.ok(result.manifest.bytes <= 220);
  assert.match(result.content, /verification/);
  assert.equal(await readFile(path.join(root, result.manifest.compression.recoveryRef), "utf8"), content);
});
...
test("observer records proven repeated work without creating or promoting a skill", async () => {
  ...
  store.create({ sessionId, provider: "codex", providerSessionId: null, parentSessionId: null, profile: "developer", profileIdentity: "profile-hash", workingDirectory: root, resumeData: null, ticketId: "T-194" });
  store.updateStatus(sessionId, "running");
  store.appendEvent(sessionId, "provider_output", "Decision: use a bounded verification checklist.\nDecision: use a bounded verification checklist.");
  store.appendEvent(sessionId, "evidence", JSON.stringify({ result: "proven", criterion: "tests pass" }));
  store.updateStatus(sessionId, "completed");
  store.close();
  try {
    const observations = await observeSession(sessionId);
    assert.ok(observations.length >= 1);
    const observation = observations.find((item) => item.signalType === "explicit-decision");
    ...
    assert.equal((await reviewObservation(observations[0].observationId, "discarded")).status, "discarded");
  } finally {
