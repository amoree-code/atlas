...
test("context rejects symlinks that escape allowed paths", async () => {
  ...
  const outside = await mkdtemp(path.join(os.tmpdir(), "atlas-context-outside-"));
  await mkdir(path.join(root, "allowed"));
  await writeFile(path.join(outside, "secret.md"), "outside secret");
  await symlink(path.join(outside, "secret.md"), path.join(root, "allowed", "linked.md"));
  const profile = validateProfile({ name: "reader", provider: "claude", role: "reader", allowedPaths: ["allowed"], contextSources: ["allowed/linked.md"] });
...
test("context enforces byte budgets for multibyte text", async () => {
  ...
  const profile = validateProfile({ name: "reader", provider: "claude", role: "reader", allowedPaths: ["arabic.md"], contextSources: ["arabic.md"] });
  const context = await buildContext(profile, root, 11);
  assert.ok(context.manifest.bytes <= 11);
  assert.ok(Buffer.byteLength(context.content.split("\n").slice(1).join("\n")) <= 11);
