...
import { invokeProviderHeadless, parseProviderStream, providerHeadlessSupport, PROVIDER_HEADLESS } from "../dist/infrastructure/providers/provider-invocation.js";
...
function resolvesOrNull(provider) {
  try { return resolveOriginalExecutable(provider); } catch { return null; }
}
...
test("unsupported providers stay explicitly gated, never silently attempted", async () => {
  for (const provider of ["hermes", "kilo", "kimi", "copilot", "openhands", "antigravity"]) {
    ...
    const result = await invokeProviderHeadless({ provider, prompt: "x", atlasSessionId: SESSION, cwd: os.tmpdir(), guard: guardFor(provider) });
...
test("no auto-approve or permission-bypass flag is ever passed to a provider", () => {
  const forbidden = ["--dangerously-skip-permissions", "--full-auto", "--dangerously-bypass-approvals-and-sandbox", "--yolo", "--auto"];
  for (const [provider, spec] of Object.entries(PROVIDER_HEADLESS)) {
    const args = spec.args("prompt");
    for (const flag of forbidden) assert.ok(!args.includes(flag), `${provider} must not pass ${flag}`);
  }
});
...
test("consent rejection (revoked/expired/wrong-session) blocks provider invocation", async () => {
  ...
  const result = await invokeProviderHeadless({
    ...
    guard: { sessionId: SESSION, classification: classifyIntent("run the build"), scope, budget: BUDGET, grant: foreignGrant },
  });
  assert.equal(result.status, "denied");
...
test("ambiguous intent cannot invoke a provider even with a grant", async () => {
  const result = await invokeProviderHeadless({
    provider: "claude", prompt: "x", atlasSessionId: SESSION, cwd: os.tmpdir(),
...
test("path/scope rejection: an invalid budget denies the invocation", async () => {
  ...
  const result = await invokeProviderHeadless({
    provider: "claude", prompt: "x", atlasSessionId: SESSION, cwd: os.tmpdir(),
    guard: { sessionId: SESSION, classification: classifyIntent("run the build"), scope, budget: { maxFiles: 0 }, grant: createGrant(SESSION, scope) },
  });
  assert.equal(result.status, "denied");
...
unixOnly("timeout produces a structured timeout result and kills the process", async () => {
  ...
  const result = await invokeProviderHeadless({
    provider: "claude", prompt: "x", atlasSessionId: SESSION, cwd: os.tmpdir(), executable: file, timeoutMs: 1_000,
    guard: guardFor("claude"),
  });
...
test("a failing provider still propagates the Atlas session pointer and parent relation", async () => {
  ...
  const result = await invokeProviderHeadless({
    provider: "claude", prompt: "x", atlasSessionId: SESSION, parentSessionId: parent, cwd: os.tmpdir(), executable: file,
    guard: guardFor("claude"),
  });
...
test("deterministic structured result: the same failure yields the same shape every time", async () => {
  const { file } = await fakeProvider("#!/bin/sh\nexit 7\n");
  const shape = (r) => ({ provider: r.provider, status: r.status, exitCode: r.exitCode, partial: r.partial, malformed: r.malformedLines });
  const first = await invokeProviderHeadless({ provider: "claude", prompt: "x", atlasSessionId: SESSION, cwd: os.tmpdir(), executable: file, guard: guardFor("claude") });
  const second = await invokeProviderHeadless({ provider: "claude", prompt: "x", atlasSessionId: SESSION, cwd: os.tmpdir(), executable: file, guard: guardFor("claude") });
...
live("live smoke: claude headless completes and returns a provider session id", async () => {
  ...
  const result = await invokeProviderHeadless({
    provider: "claude", prompt: "Reply with exactly: OK", atlasSessionId: SESSION, cwd: dir, timeoutMs: 180_000,
    guard: guardFor("claude"),
  });
...
live("live smoke: codex headless completes", async () => {
  ...
  const result = await invokeProviderHeadless({
    provider: "codex", prompt: "Reply with exactly: OK", atlasSessionId: SESSION, cwd: dir, timeoutMs: 300_000,
...
live("live smoke: gemini headless completes", async () => {
  ...
  const result = await invokeProviderHeadless({
    provider: "gemini", prompt: "Reply with exactly: OK", atlasSessionId: SESSION, cwd: dir, timeoutMs: 300_000,
