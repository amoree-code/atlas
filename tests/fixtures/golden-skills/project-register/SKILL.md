...
- `<repo>/AGENTS.md` from `~/atlas/personal/templates/project-claude.md` (template —
  ...
  differs from `docs/stack.md` — if nothing differs, say so.
- `<repo>/.claude/memory/context.md` — what it is, who it serves, where it stands, what's
...
If the remote is **not** under `example-user`, the context is personal — don't commit it:
```bash
