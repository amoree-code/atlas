...
> automatically, by `curator` and `scribe` (see the task-loop rule). By the
...
> that `scribe` wrote. Delegate the writing to `scribe` (scribe); this never
> needs architect.
...
3. **Project memory** — only if something durable changed:
   ...
   - a new rule for this codebase → `<repo>/.claude/memory/conventions.md`
   - a bug or gotcha that will bite again → `<repo>/.claude/memory/known-issues.md`
   - the project's situation moved → `<repo>/.claude/memory/context.md` and the
     **Status** section of `<repo>/CLAUDE.md`
