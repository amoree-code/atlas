...
1. **Identify the project.** In order:
   ...
   - Otherwise the most recent session record: `ls -t ~/atlas/internal/sessions/*/*/*.md | head -1`
   - Otherwise ask, offering the `active` rows from the registry.

2. **Read, in this order, stopping when you can answer:**
   - if the project uses the ticket system: `atlas context` (or `atlas context <ID>` for
   ...
   - the newest session record for that project:
     `ls -t ~/atlas/internal/sessions/*/*/*-<project>-*.md | head -1`
   - that project's rows in `~/atlas/projects/tasks.md`
