...
5. **File it** only if it has lasting value (a decision made, a tool adopted or rejected,
   a pattern worth keeping). Write `~/.ai-os/personal/knowledge/research/<topic>.md`. A throwaway
...
6. If it settles a real technical decision, also record it: cross-project →
   `~/.ai-os/personal/knowledge/decisions/`; one project → that repo's `.claude/memory/decisions.md`.
