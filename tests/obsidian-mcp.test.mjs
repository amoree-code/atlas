...
test("Obsidian MCP exposes bounded tools and requires approval for writes", async () => {
  ...
  const rejected = await handleObsidianMcpRequest({ jsonrpc: "2.0", id: 3, method: "tools/call", params: { name: "obsidian_write", arguments: { path: "note.md", content: "changed", approval: {} } } }, connection);
...
test("MCP client completes a real Atlas-to-Obsidian round trip", async () => {
  ...
  try {
    ...
    const written = await client.callTool("obsidian_write", { path: "01-Projects/round-trip.md", content: "from Atlas" }, true);
...
test("MCP rejects an approval fingerprint for a different write", async () => {
  const vaultPath = await mkdtemp(path.join(os.tmpdir(), "atlas-obsidian-mcp-fingerprint-"));
  await mkdir(path.join(vaultPath, ".obsidian"));
  const connection = { enabled: true, mode: "read-write", vaultPath };
  const result = await handleObsidianMcpRequest({ jsonrpc: "2.0", id: 4, method: "tools/call", params: { name: "obsidian_write", arguments: { path: "note.md", content: "new", approval: { approved: true, fingerprint: actionFingerprint("obsidian_write", { path: "other.md", content: "new", expectedSha256: null }) } } } }, connection);
