...
valid = run("--project", "atlas", "--session", "test-session", "--ticket", "T-053",
            "--mission", "mission/t053-s4-context-20260907",
            "--scope", "engine/cli/atlas-channel")
assert valid.returncode == 0, valid.stderr
packet = json.loads(valid.stdout)
assert packet["project_id"] == "atlas"
...
assert packet["project_path"].endswith("/projects/atlas")
assert packet["atlas_root"] == str(ROOT)

for args in (("--project", "../escape", "--session", "s", "--ticket", "T-053",
              "--mission", "mission/t053-s4-context-20260907", "--scope", "engine/cli/atlas-channel"),
             ("--project", "atlas", "--session", "../escape", "--ticket", "T-053",
              "--mission", "mission/t053-s4-context-20260907", "--scope", "engine/cli/atlas-channel"),
