...
Policy declarations moved from a top-level `policies/` to `internal/governance/policies/` on
2026-09-03. There is deliberately no `internal/governance/rules/`: no public, client-agnostic rules
...
Read `internal/governance/README.md` for the current, honest inventory — which policy file exists,
