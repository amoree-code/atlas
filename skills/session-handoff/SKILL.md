...
the last ten. It answers `ai-os lifecycle`'s `HANDOFF` decision, and is also the right
tool under `FRESH` when the destination is a different Claude account rather than the
same one restarting.

## When to run this

- `ai-os lifecycle` (or `session-end`, which calls it) returned `HANDOFF`.
...
- Never run this for routine `CHECKPOINT`/`CONTINUE` — that is what
  `ai-os tickets checkpoint` is for, and duplicating it here would just be a second
...
- Not `ai-os-handoff` (the CLI) — that is agent-to-agent task delegation with an owner
...
1. **Checkpoint first, if there's a live ticket and durable progress isn't already
   recorded.** `ai-os tickets checkpoint <ID> --note "..." --next "..."`. The packet below
   points at the ticket rather than re-stating its contents — if the ticket is stale, the
   pointer is worthless.

2. **Pull the grounding, don't restate it.** `ai-os context <ID>` (or `ai-os context` with
...
tokens, not tens of thousands. This is a guideline grounded in what `ai-os context`'s own
