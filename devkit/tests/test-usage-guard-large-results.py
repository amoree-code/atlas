...
Like `test-context-next-mode.py`, this exercises the live copy directly: engine/cli/
atlas-usage — usage's canonical implementation (T-118 retired the ~/atlas/context/
dispatch mirror this file used to exercise instead; formerly canonical since T-017) —
and is skipped, not failed, on a machine where that copy does not exist.
"""
import json
import os
import subprocess
...
LIVE_USAGE = REPO / "cli" / "atlas-usage"
...
if not LIVE_USAGE.is_file():
    print("  (skipped — no engine/cli/atlas-usage found; nothing "
          "to check)")
    print("\n0 passed, 0 failed")
    sys.exit(0)
