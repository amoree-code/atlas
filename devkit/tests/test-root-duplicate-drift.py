...
REPO = Path(__file__).resolve().parents[2]  # .../engine
...
# `atlas-*` command's own default `REPO = Path(__file__).resolve().parents[2]`
...
# proves they are actually gone, not just believed gone. (T-118 later retired the
# CORE_SHARED/CONTEXT_MECHANISM/atlas-context/atlas-observe parity mirrors this comment
# used to describe as still-pinned — see the RETIRED_FROM_CORE_CLI section below.)
...
_ATLAS = Path(__file__).resolve().parents[2].parent
...
# T-105's CORE_SHARED/CONTEXT_MECHANISM/parity-mirror contract (atlas_paths.py,
# atlas_tickets.py, atlas_lifecycle.py, atlas-context, atlas-observe, and the
# atlas-lifecycle script) held until T-118 retired it outright: ~/atlas/context/ and
# these core/cli copies are gone (backed up under runtime/backups/, not deleted
# destructively), and engine/cli/ is the sole implementation — see
# test-context-parity.py and test-cli-source-drift.py for the retirement record.
t("core/cli: the retired context/core mirror files are gone, not accidentally kept")
RETIRED_FROM_CORE_CLI = ("atlas_paths.py", "atlas_tickets.py", "atlas_lifecycle.py",
                         "atlas-context", "atlas-observe", "atlas-lifecycle")
for name in RETIRED_FROM_CORE_CLI:
    engine_f = REPO / "cli" / name
    core_f = ATLAS / "core" / "cli" / name
    chk(f"{name}: engine/cli copy exists (still the sole implementation)", engine_f.is_file())
    chk(f"{name}: atlas/core/cli copy is gone (T-118 retired it)", not core_f.is_file())
...
# `REPO = Path(__file__).resolve().parents[2]` (always engine/, since the commands
