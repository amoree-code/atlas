...
[ -f "$REPO/agentic/integrations/adapters/claude-code/rules-fragment.md" ]
chk "agentic/integrations/adapters/claude-code/rules-fragment.md exists" $?
python3 - <<'PYEOF'
import os
from pathlib import Path
m = {"__name__": "notmain",
...
AD="$REPO/agentic/integrations/adapters/claude-code/ai-memory-mounts"
...
if [ -f "$SJ" ] && grep -q 'atlas-hook' "$SJ"; then
  ...
  grep -q '\$HOME/.claude/atlas-hook agentic/integrations/adapters/claude-code/ai-guard-push' "$SJ"
...
CC_ADAPTER="$REPO/agentic/integrations/adapters/claude-code/adapter.yaml"
if [ -f "$CC_ADAPTER" ]; then
  ...
  [ -x "$REPO/agentic/integrations/adapters/claude-code/ai-response-gate" ]
...
if [ -f "$REPO/agentic/integrations/adapters/claude-code/tests/test-guard-push.py" ]; then
  python3 "$REPO/agentic/integrations/adapters/claude-code/tests/test-guard-push.py" >/dev/null 2>&1
  chk "claude-code git push guard" $?
else
  printf '  %sSKIP%s claude-code push guard tests not found\n' "$D" "$X"
fi
if [ -f "$REPO/agentic/integrations/adapters/claude-code/tests/test-response-gate.py" ]; then
  python3 "$REPO/agentic/integrations/adapters/claude-code/tests/test-response-gate.py" >/dev/null 2>&1
