...
3. **Ground it in the actual stack.** Read
   `docs/stack.md` and, if a specific project prompted this,
   its `CLAUDE.md`. A recommendation that ignores TypeScript · React · Node is not useful here.
...
5. **File it** only if it has lasting value (a decision made, a tool adopted or rejected,
   a pattern worth keeping). Write `~/atlas/personal/knowledge/research/<topic>.md`. A throwaway
...
6. If it settles a real technical decision, also record it: cross-project →
   `~/atlas/personal/knowledge/decisions/`; one project → that repo's `.claude/memory/decisions.md`.
