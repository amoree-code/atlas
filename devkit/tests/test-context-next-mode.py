...
`atlas-context`'s canonical implementation lives at engine/cli/atlas-context (T-118
retired the ~/atlas/context/ dispatch mirror this file used to exercise instead; same file
T-034's `--mode planning` test exercises). This file tests that live file directly, against
a disposable ATLAS_HOME/ATLAS_HOME, so nothing here touches the real workspace.
...
LIVE_CONTEXT = REPO / "cli" / "atlas-context"
CLI = REPO / "cli"
...
if not LIVE_CONTEXT.is_file():
    print("  (skipped — no engine/cli/atlas-context found; nothing "
          "to check)")
    print("\n0 passed, 0 failed")
    sys.exit(0)
