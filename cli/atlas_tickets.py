"""atlas_tickets — reading the durable ticket records, for Python callers.

One implementation of "what does this ticket say", used by `atlas tickets` and by
`atlas context`. Two readers of the same records is how two views of one status start
...
BEGIN = "<!-- atlas:tickets:begin -->"
END = "<!-- atlas:tickets:end -->"
...
def parse_ticket_id(ticket_id):
    """(generation, number) for an id in one of the two atlas/Atlas ticket generations,
...
CHECKBOX_RE = re.compile(r"^-\s+([\"'])\[( |x)\]\s+.+\1\s*$")
...
# by `atlas tickets archive`. `archive/` is a container, never itself a ticket id, and
...
# --- Duplicate Ticket Guard (T-100) --------------------------------------------------------
# Deterministic keyword overlap only — no AI call, no embedding, no vector store. A title is
# reduced to its lowercase words minus a small set of pure grammatical connectors (never a
# domain word like "fix" or "add" — those carry real signal about what the ticket does), and
# two titles are compared by Jaccard similarity over those keyword sets. This is intentionally
# coarse: it exists to catch the ticket that is obviously the same work reworded, not to judge
# relatedness with any nuance a human would still need to weigh in on.
_DUPLICATE_WORD_RE = re.compile(r"[a-z0-9]+")
DUPLICATE_STOPWORDS = frozenset({
    "a", "an", "the", "and", "or", "for", "of", "to", "in", "on", "at", "with", "by",
    "from", "is", "are", "be", "this", "that",
})
# Above this Jaccard score, two titles are treated as the same work until an owner says
# otherwise — this is the line `cmd_new` blocks on unless `--force --reason` is given.
DUPLICATE_HIGH_CONFIDENCE = 0.6


def normalize_keywords(title):
    """A title reduced to its deterministic keyword set: lowercase words, stopwords
    dropped, nothing stemmed or inferred. Same input always yields the same set."""
    words = _DUPLICATE_WORD_RE.findall((title or "").lower())
    return {w for w in words if w not in DUPLICATE_STOPWORDS}


def duplicate_score(keywords_a, keywords_b):
    """Jaccard similarity — |intersection| / |union| — between two keyword sets. 0.0 when
    either side has nothing left after stopwords, since there is nothing to compare."""
    if not keywords_a or not keywords_b:
        return 0.0
    union = len(keywords_a | keywords_b)
    return len(keywords_a & keywords_b) / union if union else 0.0


def duplicate_candidates(title, all_tickets, exclude_id=None):
    """`title` scored against every live candidate ticket (`CANDIDATE_STATES` — todo,
    active, paused, blocked; never archived, done or cancelled — a finished or retired
    ticket is not "already covers the same work"). Returns `(ticket, score, keywords)`
    triples for every ticket that shares at least one keyword, sorted by score descending;
    `keywords` is the overlap itself, so a caller can explain the match rather than just
    assert it."""
    ...
    for t in all_tickets:
        if exclude_id and t["id"] == exclude_id:
            continue
        if t["is_archived"] or t["state"] not in CANDIDATE_STATES:
            continue
        other = normalize_keywords(t["title"])
        score = duplicate_score(keywords, other)
        if score > 0:
...
# One implementation, same reason the rest of this module is one implementation: `atlas
# tickets next` and `atlas context`'s recommended-next-actions view must never compute two
...
def explain_ticket(t, goal, today):
    ...
    both `atlas tickets next` and `atlas context`'s next-actions view get an answer from,
...
def render_recommendation(buckets, why=False):
    """The one text rendering of a `classify_and_rank` result — shared by `atlas tickets
    next` and `atlas context`'s recommended-next-actions view, so the same underlying
...
# before-the-fact refusal used by `atlas tickets new`/`promote`, in `atlas-tickets`.
#
...
# EXTENSION split). `atlas tickets promote` is the one path that moves a ticket OUT of
...
def lifecycle_issues(meta, by_id):
    ...
    `atlas tickets new`/`promote` refuses outright, so an incomplete or invalid ticket can
    never enter the system in the first place. Reuses the exact enum tables and formats
    `atlas tickets doctor` already checks (`STATES`, `CLASSES`, `RELATIONS`, `PRIORITIES`,
