"""aios_coordination — T-050-S6/S6-R1: ticket leases, file ownership claims, and the
additive coordination-state layer, exactly as authorized by
`projects/ai-os/tickets/T-050/T-050-S5-conflict-protection-design.md`, with the concurrency
and enforcement fixes from `T-050-S6-R1-implementation.md`.

This is a library, imported by `cli/ai-os-coordinator`, never run directly. It writes durable
...
(`runtime/coordination/claims/*.json`) — the same root `aios_tickets.atlas_runtime_dir()`
...
# --- home / storage roots ---------------------------------------------------------------
def atlas_home():
    """${ATLAS_HOME:-~/atlas} — the same resolution `aios_tickets.atlas_runtime_dir()`
...
def runtime_claims_dir():
    ...
    `aios_tickets.atlas_runtime_dir()`), never a new runtime root."""
...
def resolve_scope_paths(scope):
    ...
    supported representation today (the existing `SCOPE_SAFE` shape `cli/ai-os-coordinator`
...
def verify_dispatch_conflict_protection(task_dir, task_id, scope, lease_id, client, session):
    ...
    `ai-os handoff send` — never applied to that command directly, and never to any other
