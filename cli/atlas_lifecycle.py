"""atlas_lifecycle — one deterministic reading of what a session should do next.

The decision is CONTINUE · CHECKPOINT · COMPACT · FRESH · HANDOFF, and this module is the
only place it is made. `atlas lifecycle` gathers the evidence and renders it;
`atlas usage --guard` maps its own measured findings through the same function. Two
...
`atlas usage` imports this module, so this module must import nothing back.
"""
...
# checked against each other by `atlas lifecycle effort --doctor`.
...
def decide(ev):
    ...
    def out(decision, sequence, why, certainty, consider=()):
        return {
            ...
            "ticket": ev["ticket"],
            "resume_with": f"atlas context {ev['ticket']}" if ev["ticket"] else "atlas context",
    ...
    #    fix is `atlas observe`, not a fresh session.
    if HIGH_TOOL_NOISE in fired:
        return out(CHECKPOINT, [CHECKPOINT],
                   "large raw tool results are most of what this context holds — "
                   "checkpoint, and run them through `atlas observe`",
                   "recommended", [COMPACT])
...
def guard_lifecycle(finding_code, has_record=True, reconstructable=True):
    ...
    `atlas usage` measures transcripts. It cannot know whether reasoning is unresolved,
    ...
    if finding_code == "large_results":
        return (CHECKPOINT, "run large output through `atlas observe`, then checkpoint")
