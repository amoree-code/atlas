"""atlas_extensions — isolated discovery/resolution over $ATLAS_HOME/extensions/ (T-029).
...
in isolation first"). It is deliberately NOT wired into `ai-sync` or `atlas-render`'s live
behavior yet — both keep reading their pre-existing legacy sources
(`<repo>/skills/` for `atlas-render`) until an explicit, separately-approved cutover step.
...
def _extensions_root(atlas_home_path):
    ...
    if not root.is_dir():
        ...
        # silent read of <repo>/skills or ~/atlas/internal/extensions instead.
...
def render_source_for(kind, atlas_home_path=None):
    ...
    This is the isolated equivalent of what `atlas-render`'s `SKILLS` constant, or an
    equivalent future `AGENTS`/`MCP` constant, would resolve to if pointed at Atlas. It is
    exercised by tests to prove the resolution logic is correct; `atlas-render` itself is
