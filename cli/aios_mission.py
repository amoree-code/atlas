"""aios_mission — T-051-S1: the Mission Contract and durable Mission Run State layer only.

This is a library, imported by `cli/ai-os-mission`, never run directly. It writes durable
...
ticket directory `aios_tickets`/`ai-os-paths` already resolve, never a second authority or a
new runtime root.
...
released in a `finally` — the same technique `cli/aios_coordination.py` uses (S6-R1), kept as
...
PATHS_RESOLVER = CLI_DIR / "ai-os-paths"


def adapters_dir():
    """Read at call time, never cached at import — the same posture as `atlas_home()` below,
    and required for `AI_OS_ADAPTERS` to be honored by a caller (a test fixture) that sets it
    after this module has already been imported."""
    return Path(os.environ.get("AI_OS_ADAPTERS", CLI_DIR.parent / "adapters"))
...
# regex object (not imported from cli/aios_coordination.py) for the same independence reason
...
# narrow family `cli/ai-os-handoff` already scans for — not imported from that protected
...
def resolve_task_dir(task_id):
    ...
    resolver every other command already uses (`ai-os-paths ticket <id>`) — read-only,
...
def canonicalize_scope(raw_path):
    ...
    Atlas root. Identical rule set to `cli/aios_coordination.canonicalize_path`, kept as an
...
# resolver function (`ai-os-handoff.resolve_transport`) the coordinator already uses.
# ============================================================================================

def _load_sibling(name):
    """Load a sibling CLI file as a library module, never as `__main__` — the identical
    technique `cli/ai-os-coordinator._load_sibling` already uses to reuse `cli/ai-os-adapter`
    and `cli/ai-os-handoff` without forking a second copy of either. Kept as an independent
    ...
    if not src.exists():
        refuse(f"cli/{name} is missing; mission routing reuses it and cannot run without it",
               code=5)
    mod_name = "_aios_mission_" + name.replace("-", "_").replace(".", "_")
...
def load_adapter(client):
    ...
    adapter_mod = _load_sibling("ai-os-adapter")
    try:
...
def resolve_role(task_dir, root_task_id, mission_id, role):
    ...
    hoff = _load_sibling("ai-os-handoff")
...
def _require_gate(value, gates):
    ...
    (`cli/ai-os-handoff.GATES`) rather than inventing a second one."""
    if value is None or not str(value).strip():
...
# is hard-coded to one historical trial directory (`projects/ai-os/tickets/AIOS-012`) and
...
def mission_pilot_transport_argv(task_dir, mission_id, handoff_id,
    ...
    `ai-os-handoff` registry — no second registry, no new registry file) and this handoff's
...
def _coord():
    """Load `cli/aios_coordination.py` as a library module — the identical `_load_sibling`
    technique this file already uses for `cli/ai-os-handoff`/`cli/ai-os-adapter`, and the
    identical technique `cli/ai-os-coordinator._coord()` uses for the same file. This is the
    ...
    `cli/aios_coordination.py` or to any T-050 record directly — every mutation below is a
    plain call into that file's own, unmodified, already-tested functions."""
    return _load_sibling("aios_coordination.py")
...
def _execute_sub_key(idempotency_key, purpose):
    ...
    literal string. Identical derivation to `cli/ai-os-coordinator._sub_key` (independent copy,
...
def mission_execute(task_dir, root_task_id, mission_id, handoff_id, executor_client,
    ...
    existing, unmodified `cli/aios_coordination.py` library (never a duplicated lease/claim
    ...
    with _guard(handoff_dir(task_dir, mission_id, handoff_id) / ".execute.lock",
        ...
        # The actual `subprocess.run` call lives in `cli/ai-os-handoff.run_transport_subprocess`
        # — not inline here — so that aios_mission.py itself keeps making exactly the one
        # subprocess call every prior T-051 slice's own frozen test already asserts it makes
        # (to `ai-os-paths`, for ticket resolution). This is the module's own established
